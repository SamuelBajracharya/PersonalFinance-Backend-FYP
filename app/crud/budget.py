...
def _update_remaining_budget(db: Session, budget: Budget):
    ...
    current_spending_decimal = (
        Decimal(current_spending) if current_spending is not None else Decimal(0)
    )
...
def get_budget_by_category_and_user_and_date(
    db: Session, user_id: str, category: str, start_date: date, end_date: date
):
    budget = (
        db.query(Budget)
        .filter(
...
def create_budget(db: Session, budget: BudgetCreate, user_id: str):
    # Enforce budget creation rule using projected 30-day spend pace for this month.
    current_month_spending = get_total_spending_for_category_and_month(
        db, user_id, budget.category, budget.start_date.year, budget.start_date.month
    )
...
def evaluate_budget_completion(db: Session, budget: Budget, user: User) -> bool:
    ...
    if success:
        ...
        # Issue one achievement voucher aligned with user's current XP tier
        try:
